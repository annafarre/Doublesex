...
v <- voom(dge.filt1, design, plot=TRUE)
...
v.loess <- voom(dge.filt1, design, normalize.method="cyclicloess", plot=TRUE)
...
v.quant <- voom(dge.filt1, design, normalize.method="quantile", plot=TRUE)
...
plotMDS(v.loess,pch = v$targets$population, col=as.numeric(as.factor(v$targets$caste)))
plotMDS(v.loess,pch = 16, col=as.numeric(as.factor(v$targets$population)))
...
ex.caste.quant <- diffSplice(fit.caste, geneid="GeneID")
```
...
```{r results="hold"}
#ex <- diffSplice(fit, geneid="GeneID")
ex.sex <- diffSplice(fit.sex, geneid="GeneID")
```
