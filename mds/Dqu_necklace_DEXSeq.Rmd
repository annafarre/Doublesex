...
```{r root.dir=TRUE}
dataFile = read.table('/Users/afarre/Unimelb/Doublesex/Dquadriceps/necklace/relST_genomeST.psl', header = FALSE, sep = "\t", skip = 5, stringsAsFactors = F)
```
...
```{r rows.print=5}
sample_id<-read.table("/Users/afarre/Unimelb/Doublesex/Dquadriceps/data_info/Dqu_SRR_Acc_List.txt", col.names = "sampleID", stringsAsFactors = F )

#exclude a pooled sample
sample_id <- filter(sample_id, sampleID!="SRS661877")

info_samples<-read.table("/Users/afarre/Unimelb/Doublesex/Dquadriceps/data_info/Dqu_SraRunTable.txt", sep = "\t", header = T,  stringsAsFactors = F )
info_samples <- filter(info_samples, SRA_Sample!="SRS661877")
rownames(info_samples) <- info_samples$Run
...
info_samples[info_samples$Run %in% dplyr::filter(info_samples, social_status=="reproductive")$Run,]$casteName <- "Queen"
info_samples[info_samples$Run %in% dplyr::filter(info_samples, social_status=="non-reproductive")$Run,]$casteName <- "Worker"

info_samples[info_samples$Run %in% dplyr::filter(info_samples, social_status=="reproductive")$Run,]$caste <- "1"
info_samples[info_samples$Run %in% dplyr::filter(info_samples, social_status=="non-reproductive")$Run,]$caste <- "2"


data_info <- dplyr::select(info_samples, Run, casteName)
...
counts <- read.table("/Users/afarre/Unimelb/Doublesex/Dquadriceps/necklace/block.counts",sep="\t",header=TRUE,stringsAsFactors=F)
names(counts) <- gsub(".*(SRR.*)\\.bam","\\1", names(counts))
counts <- dplyr::select(counts, -SRR1519118)
...
```{r rows.print=10}
data_infoLimma <- dplyr::select(info_samples, Run, caste)
row.names(data_infoLimma) <- data_infoLimma$Run
data_infoLimma$caste <- as.factor(data_infoLimma$caste)

design <- model.matrix(~ 0 + data_infoLimma$caste)
row.names(design) <- data_infoLimma$Run
...
adjPperGene[adjPperGene$gene=="gene5793:MSTRG.5916",]
res[grep("MSTRG.5916",res$featureID ),]
```

Save the Rdata and tables 

```{r}
save(dxd, res, pgq, file = "/Users/afarre/Unimelb/Doublesex/R/DEXSeq/Dqu_DEXseq.Rdata")
write.table(tmp, file = "/Users/afarre/Unimelb/Doublesex/R/DEXSeq/Dqu_DEXseq_byGene.txt", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(DEXSeqResults(dxd), file="/Users/afarre/Unimelb/Doublesex/R/DEXSeq/Dqu_DEXseq_byExon.txt", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t" )
```
...
adjPperGene$qmstrg <- sub(".*(MSTRG\\.\\d*).*", "\\1", adjPperGene$gene)

#Find the names of the genes is Apis 
diffMatches.caste <- adjPperGene %>%
...
write.table(diffMatches.caste,"/Users/afarre/Unimelb/Doublesex/R/DEXSeq/dsCaste_dexseq_dqu.txt", col.names = TRUE)
