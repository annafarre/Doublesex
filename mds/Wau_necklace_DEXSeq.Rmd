...
```{r rows.print=10}
data_infoLimma <- dplyr::select(info_samples, Run_s, caste, population)
row.names(data_infoLimma) <- data_infoLimma$Run_s
data_infoLimma$caste <- as.factor(data_infoLimma$caste)

design <- model.matrix(~ 0 + data_infoLimma$caste + data_infoLimma$population)
...
lib.size$caste <- data_info[lib.size$sample,]$condition
...
ex_gene_sample$caste <- data_info[ex_gene_sample$sample,]$condition
...
# dgeQW <- dge.filt0[,data_info[data_info$condition!="male",]$sample]
nshort <- 1000
dgeQW <- head(count_matrix[,data_info[data_info$condition!="male",]$sample], nshort)

#small dataset to test DEXSeq models
# count_matrix <- head(data.matrix(dgeQW$counts), 100)
# gene_ids <- head(dgeQW$genes$GeneID, 100)
# exon_ids <- head(dgeQW$genes$exon_ids, 100)

# count_matrix <- data.matrix(dgeQW$counts)
...
write.table(diffMatches.caste,"/Users/afarre/Unimelb/Doublesex/R/DEXSeq/dsCaste_dexseq_wau.txt", col.names = TRUE)
