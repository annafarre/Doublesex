...
```{r}
dataFile = read.table('/Users/afarre/Unimelb/Doublesex/Aechinatior/necklace/relST_genomeST.psl', header = FALSE, sep = "\t", skip = 5, stringsAsFactors = F)
```
...
```{r  rows.print=5}
counts <- read.csv("/Users/afarre/Unimelb/Doublesex/Aechinatior/necklace/block.counts", sep = "\t", skip = 1, stringsAsFactors = F)
...
colnames(design)<- c("gyne", "large_worker", "small_worker")
...
contrast.matrix.sex <- makeContrasts(gyne-(large_worker+small_worker)/2, levels=design)
...
percentMapped=100-3209/4496*100
...
plotSplice(ex.sex, coef = 1, geneid = "gene3552:gene3553:MSTRG.4260")
...
#diff$genes=="gene13749:MSTRG.15287"
pvalues_dsx.sex <- ex.sex$p.value[ex.sex$gene.firstexon["gene3552:gene3553:MSTRG.4260"]:ex.sex$gene.lastexon["gene3552:gene3553:MSTRG.4260"],]
...
Aec_diffMatches.sex <- diffMatches.sex
Aec_diffMatches.caste <- diffMatches.caste
Aec_intersect_diffMatches <- intersect((unlist(diffMatches.sex)), (unlist(diffMatches.caste)))
...
write.table(x = as.data.frame(unlist(Aec_diffMatches.caste)), file = "../../R/Aec_diffMatches_casteWorkers.txt", col.names = TRUE, row.names = TRUE)
write.table(x = as.data.frame(unlist(Aec_diffMatches.sex)), file = "../../R/Aec_diffMatches_casteGyneWorker.txt", col.names = TRUE, row.names = TRUE)
...
n <- 200000
nshort <- 20
a <- topSplice(ex.sex, coef=1, test="simes", number=n,  FDR = 0.5)

a_genes <- unlist(strsplit(a$GeneID, ":"))[grep("gene*", unlist(strsplit(a$GeneID, ":")))]
dcaste <- sapply(a_genes, function(e){aec_gtf[mcols(aec_gtf)$ID==e]$Name})

# ashort <- topSplice(ex.caste, coef=1, test="simes", number=nshort,  FDR = 0.5)
# dcasteshort <- sapply(unlist(strsplit(ashort$GeneID, ":"))[grep("gene*", unlist(strsplit(ashort$GeneID, ":")))], function(e){aec_gtf[mcols(aec_gtf)$ID==e]$Name})

nay <- na.omit(as.data.table(aec_gtf), cols="gene")
...
write.table(x = int_genes, file = "/Users/afarre/Unimelb/Doublesex/R/Aec_AS_int_genes.txt", col.names = TRUE, row.names = TRUE)
