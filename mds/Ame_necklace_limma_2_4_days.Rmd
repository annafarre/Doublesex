---
title: "Ame_necklace_limma_2_4day"
author: "Anna Orteu"
date: "24/04/2018"
output: 
  html_document:
...
counts <- cbind(counts[,1:6],counts[,22:57])
counts
...
sample_table_SRP068487 <- dplyr::select(info_samples_SRP068487, age, Run_s, caste, sex, study, ageName, casteName, sexName)
...
design <- model.matrix(~ 0 + data_info$caste + data_info$age)
...
colnames(design)<- c("queen", "worker", "male", "fourDay")
...
dge$samples[as.data.frame(design)$fourDay==0,]$age <- "2"
...
write.table(x = as.data.frame(unlist(Ame_diffMatches.sex)), file = "~/Unimelb/Doublesex/R/Ame_diffMatches_2_4_days_sex.txt", col.names = TRUE, row.names = TRUE)
write.table(x = as.data.frame(unlist(Ame_diffMatches.caste)), file = "~/Unimelb/Doublesex/R/Ame_diffMatches_2_4_days_caste.txt", col.names = TRUE, row.names = TRUE)
