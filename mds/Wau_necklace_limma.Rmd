...
percentMapped=100-2914/4066*100
...
Wau_diffMatches.sex <- diffMatches.sex
Wau_diffMatches.caste <- diffMatches.caste
Wau_intersect_diffMatches <- intersect((unlist(diffMatches.sex)), (unlist(diffMatches.caste)))

write.table(x = as.data.frame(unlist(Wau_diffMatches.sex)), file = "../../R/Wau_diffMatches_sex.txt", col.names = TRUE, row.names = TRUE)
...
dcaste <- sapply(unlist(strsplit(a$GeneID, ":"))[grep("gene*", unlist(strsplit(a$GeneID, ":")))], function(e){wau_gtf[mcols(wau_gtf)$ID==e]$Name})

ashort <- topSplice(ex.caste, coef=1, test="simes", number=nshort,  FDR = 0.5)
dcasteshort <- sapply(unlist(strsplit(ashort$GeneID, ":"))[grep("gene*", unlist(strsplit(ashort$GeneID, ":")))], function(e){wau_gtf[mcols(wau_gtf)$ID==e]$Name})

nay <- na.omit(as.data.table(wau_gtf), cols="gene")
...
write.table(x = int_genes, file = "/Users/afarre/Unimelb/Doublesex/R/Wau_AS_int_genes.txt", col.names = TRUE, row.names = TRUE)

d <- topSplice(ex.sex, coef=1, test="simes", number=n,  FDR = 0.5)

dsex <- sapply(unlist(strsplit(d$GeneID, ":"))[grep("gene*", unlist(strsplit(d$GeneID, ":")))], function(e){wau_gtf[mcols(wau_gtf)$ID==e]$Name})




grid.newpage()
venn.plot <- draw.pairwise.venn(area1 = length(unlist(dsex)), 
                   area2 = length(unlist(dcaste)), 
                   cross.area = length(intersect((unlist(dsex)), (unlist(dcaste)))), 
                   category = c("DS sex", "DS caste"))
```
...
dme_gtf[dme_gtf$gene_idFB==dme_dsx_targets$Gene_Flybase_ID[1],]

dme_gtf[dme_gtf$gene_idFB=="FBgn0003742",]
