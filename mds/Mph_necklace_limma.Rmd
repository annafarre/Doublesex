...
info_samples$sample_title_s
info_samples$sample_name_s
```
...
DRP002497_COL1 <- info_samples[grep("^PHARAO\\..*COL1$", info_samples$sample_name_s, perl = T),]$Run_s
DRP002497_COL2 <- info_samples[grep("^PHARAO\\..*COL2$", info_samples$sample_name_s, perl = T),]$Run_s
...
info_samples[info_samples$Run_s %in% DRP002877,]$study <- "DRP002877"
info_samples[info_samples$Run_s %in% DRP002497_COL1,]$study <- "DRP002497"
info_samples[info_samples$Run_s %in% DRP002497_COL2,]$study <- "DRP002497"
...
info_samples[info_samples$Run_s %in% DRP002877,]$colony <- "1"
info_samples[info_samples$Run_s %in% DRP002497_COL1,]$colony <- "2"
info_samples[info_samples$Run_s %in% DRP002497_COL2,]$colony <- "3"


data_info <- dplyr::select(info_samples, Run_s, study, caste, colony)
data_info <- data_info[data_info$Run_s!="DRR029634",]
row.names(data_info) <- data_info$Run_s

design <- model.matrix(~ 0 + data_info$caste + data_info$study)
...
colnames(design)<- c("queen", "worker", "DRP002877")
...
# dge$samples$colony <- "1"
# dge$samples[as.data.frame(design)$col2==1,]$colony <- "2"
# dge$samples[as.data.frame(design)$col3==1,]$colony <- "3"

dge$samples$study <- ""
dge$samples[as.data.frame(design)$DRP002877==1,]$study <- "DRP002877"
dge$samples[as.data.frame(design)$DRP002877==0,]$study <- "DRP002497"
...
Mph_diffMatches.sex <- diffMatches.sex
Mph_diffMatches.caste <- diffMatches.caste
Mph_intersect_diffMatches <- intersect((unlist(diffMatches.sex)), (unlist(diffMatches.caste)))

write.table(x = as.data.frame(unlist(Mph_diffMatches.sex)), file = "../R/Mph_diffMatches_sex.txt", col.names = TRUE, row.names = TRUE)
write.table(x = as.data.frame(unlist(Mph_diffMatches.caste)), file = "../../R/Mph_diffMatches_caste.txt", col.names = TRUE, row.names = TRUE)
# identical(Wau_diffMatches.caste, Mph_diffMatches.caste)
```